...
if strcmp(bmiMode, "initialize") || strcmp(runMode, "full")
    ...
    OutputPath = fullfile(OutputPath, datestr(now, "yyyymmdd_HHMM"),filesep);
...
if strcmp(bmiMode, 'update') || strcmp(runMode, 'full')
    ...
    while KT < endTime
        ...
        if IRPT1 == 0 && IRPT2 == 0 && SoilVariables.ISFT == 0
            ...
            if calculate
                ...
                switch options.calc_ebal
                    case 1
                        [iter, fluxes, rad, thermal, profiles, soil, RWU, frac, WaterStressFactor, WaterPotential, TestPHS, elecFluo] ...
        ...
        n_col = io.output_data_binary(file_ids, k, xyt, rad, canopy, ScopeParameters, vi, vmax, options, fluxes, ...
                                      ...
                                      Sim_qvt, Sim_qla, Sim_qva, Sim_qtot, ForcingData, RS, RWUs, RWUg, elecFluo);
        fclose("all");
    end
end
